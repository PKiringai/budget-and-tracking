...
public class BudgetController {
    ...
    @Operation(summary = "Create Budget",
            description = "Create a new budget for a customer with multiple categories")
    public ResponseEntity<ApiResponse<CustomerBudgetResponse>> createBudget(
            @Valid @RequestBody BudgetCreateRequest request) {

        log.info("POST /api/v1/budgets - Creating budget for User: {}", request.getUserId());

        Budget savedBudget = budgetService.createBudget(request);

        CustomerBudgetResponse budgetData = CustomerBudgetResponse.builder()
                .accountNumber("01100611339002") // map properly from user details
                .accountName("ELLIANNE SULAA") // map properly from user details
                .phoneNumber("254123456789") // map properly from user details
                .savingsPeriod(String.valueOf(savedBudget.getPeriodUnit()))
                .maturityDate(savedBudget.getEndDate() != null ? savedBudget.getEndDate().toString() : null)
                .startDate(savedBudget.getStartDate() != null ? savedBudget.getStartDate().toString() : null)
                .endDate(savedBudget.getEndDate() != null ? savedBudget.getEndDate().toString() : null)
                .frequency(String.valueOf(savedBudget.getPeriodUnit()))
                .reminder("MONDAY")
                .transactionId(request.getMessageId())
                .build();

        return ResponseEntity
                .status(HttpStatus.CREATED)
                .body(ApiResponse.success("Budget created successfully", budgetData));
    }
...
//            @Valid @RequestBody BudgetCreateRequest request) {
//
//        log.info("POST /api/v1/budgets - Creating budget for CIF: {}, Categories: {}",
//                request.getCifId(),
//                request.getCategories().stream()
//                        .map(BudgetCreateRequest.BudgetItemRequest::getCategoryCode)
//                        .toList());
//
//        Budget savedBudget = budgetService.createBudget(request);
//
//        CustomerBudgetResponse response = CustomerBudgetResponse.builder()
//                .accountNumber("01100611339002") // TODO map properly
//                .accountName("ELLIANNE SULAA")   // TODO map properly
//                .phoneNumber("254123456789")     // TODO map properly
//                .savingsPeriod(savedBudget.getPeriodUnit())
//                .maturityDate(savedBudget.getEndDate().toString())
//                .startDate(savedBudget.getStartDate().toString())
//                .endDate(savedBudget.getEndDate().toString())
//                .frequency(savedBudget.getPeriodUnit())
//                .reminder("MONDAY")
//                .transactionId(request.getMessageId())
//                .build();
//
